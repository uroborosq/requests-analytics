...
class PlotAverageTime(object):
    def __init__(self, s: pd.Series, title: str):
        fig, axes = subplots(nrows=1, ncols=1, num='Скорость закрытия заявок')
        axes.plot(s.keys(), [td / np.timedelta64(1, 'D') for td in s.values], marker='o')
        rcParams.update({'font.size': 8})

        for key, row in s.items():
            axes.annotate("  " + str(row.days) + " дней", [key, row.days])
...
class DoneWaitReceive(object):
    def __init__(self, data: list[pd.Series | pd.DataFrame], title):
...
class Pie(object):
    def __init__(self, data: pd.Series, title: str, suptitle: str):
        fig, axes = subplots(nrows=1, ncols=1, num=title)
        rcParams.update({'font.size': 8})
...
class SLPlot:
    def __init__(self, data: pd.Series, title: str, limit_level: float):
        ...
        axes.axhline(y=limit_level, color='r', linestyle='--')

        axes.set_title(title)
        axes.set_xlabel('Сотрудники, ФИО')
        axes.set_ylabel('SL, %')
        ...
        for key, row in data.items():
            axes.annotate("{:.1f}".format(row) + "%", [key, row + 1], ha='center')
